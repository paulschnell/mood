    ...
    pub fn new(path: &str) -> Self {
        ...
        for sector in map_json["sectors"].members_mut() {
            ...
            for corner in sector["corners"].members_mut() {
                ...
                data.vertices.push([x, floor - 0.5, z]); // floor
                data.vertices.push([x, ceiling - 0.5, z]); // ceiling
...

            {
                ...
                for i in 0..(data.corners.len() as u32 - 1) {
                    if !gates_index.contains(&i) {
                        data.indices.push(i * 2);
                        data.indices.push(i * 2 + 1);
                        data.indices.push(i * 2 + 2);
                        data.indices.push(i * 2 + 1);
                        data.indices.push(i * 2 + 3);
                        data.indices.push(i * 2 + 2);
                    }
                }
                // between last and first element
                if !gates_index.contains(&(data.corners.len() as u32 - 1)) {
                    data.indices.push((data.corners.len() as u32 - 1) * 2);
                    data.indices.push((data.corners.len() as u32 - 1) * 2 + 1);
                    data.indices.push(0);
                    data.indices.push((data.corners.len() as u32 - 1) * 2 + 1);
                    data.indices.push(1);
                    data.indices.push(0);
            ...
            for i in 0..(data.corners.len() as u32 - 2) {
                data.indices.push(0);
                data.indices.push(i * 2 + 2);
                data.indices.push(i * 2 + 4);
            }
            for i in 0..(data.corners.len() as u32 - 2) {
                data.indices.push(1);
                data.indices.push(i * 2 + 5);
                data.indices.push(i * 2 + 3);
            }
...
impl Renderable for Map {
    fn create(&mut self) {
        for sector in self.data.sectors.clone().into_iter() {
            let vertices = sector.vertices;
            let indices = sector.indices;
            unsafe {
                ...
                gl::BufferData(
                    ...
                    self.data.sectors[0]
                        ...
                        .as_ptr_range()
                        .end
                        .offset_from(self.data.sectors[0].vertices.as_ptr_range().start)
                        * std::mem::size_of::<Vertex>() as isize,
                    self.data.sectors[0].vertices.as_ptr().cast(),
                    gl::STATIC_DRAW,
                );
                ...
                gl::BufferData(
                    ...
                    self.data.sectors[0]
                        ...
                        .as_ptr_range()
                        .end
                        .offset_from(self.data.sectors[0].indices.as_ptr_range().start)
                        * std::mem::size_of::<u32>() as isize,
                    self.data.sectors[0].indices.as_ptr().cast(),
                    gl::STATIC_DRAW,
                );
    ...
    fn render(&self, shaders: &Shader) {
        ...
        unsafe {
            ...
            gl::DrawElements(
                gl::TRIANGLES,
                self.data.sectors[0].indices.len() as i32,
                gl::UNSIGNED_INT,
                0 as _,
            );
        }
